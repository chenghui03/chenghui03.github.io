---
{"publish":true,"created":"2025-12-23T21:22:24.906+08:00","modified":"2025-12-27T12:15:52.278+08:00","cssclasses":""}
---


## 基因组注释：构建从序列到功能的映射坐标系

基因组学的任务是在海量的 $A, T, C, G$ DNA序列中挖掘人类可理解的生物学功能。基因组注释（Genome Annotation）将线性序列和不同层面的生物学意义关联起来。genome注释精确度直接决定了后续转录组定量、变异检测以及进化分析的信噪比。
...
### The backgroupd: Repeats



Tandem Repeats
	**Satellite DNA)**：通常位于着丝粒（Centromere）和端粒（Telomere)
	**Microsatellites/STRs**: 法医和群体遗传学
	rRNA cluster
	
Interspersed Repeats/TE( element)
	散布在基因组各处的重复序列，通常由转座子产生
	**LINEs/SINEs**
	

在人类genome中, repeat所占比例约为50%


> [!NOTE] 重复出现gene是重复序列吗?
> 重复的gene源于基因复制事件（Duplication event），随时间推移，这些拷贝可能会保留相同功能，也可能分化出新功能，或者变成假基因（Pseudogenes）.例如在人类中NOTCH2NL存在多个拷贝.
> 而repeats通常不产生有功能的蛋白质（虽然有些转座子包含编码酶的基因，但通常被视为“寄生”序列而非宿主基因).
> 
> 二者一定程度上存在交集: 核糖体 RNA（rRNA）基因在基因组中通常以**串联重复**（Tandem Repeats）的形式存在，排列非常紧密10。在这种情况下，它们既是基因，也是重复序列结构。

### not repeats

![[attachments/Pasted image 20251227115437.png|500]]
(Shafee & Lowe, 2016)

DNA 到前体 mRNA（pre-mRNA）再到成熟 mRNA 的过程
	exon: 保留在mRNA中的序列
	intron: 从pre mRNA被剪切, 不在mRNA中的序列

从mRNA翻译为蛋白质
	CDS: 被翻译成蛋白质对应的核酸序列
	ORF: 以起始密码子开始, 终止密码子结束的核酸序列(他是CDS的超集)
	UTR: 不被翻译成蛋白质的核酸序列 

> [!NOTE] CDS/UTR/Exon 的概念适用范围
> 这些概念既可以指代基因组（Genome/DNA）上的特定区域，也可以指代转录本（mRNA）上的序列片段. 一般来说genome被当作相对稳定的绝对坐标系, 所以具体指代例如xxx exon时候可以说明其在染色体上位置
...
![](https://upload.wikimedia.org/wikipedia/commons/thumb/c/cf/Genome_Annotation_Timeline.svg/960px-Genome_Annotation_Timeline.svg.png)
## 非模式生物的组装困境
